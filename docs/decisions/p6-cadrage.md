...
| **P6-2** ✅ **LIVRÉ (rc.93/96/97)** | **Brancher le checkout déclaratif (approche A) + admin** | ✅ **Abonnement** (`UpgradeView`, rc.93) ✅ **Commercial back** (`PaymentMethodCatalog` + `POST /api/payments` accepte `provider`, rc.96) ✅ **Commercial front** (sélecteur marché dans `OrderDetailView`, repli gracieux, rc.97). Reste optionnel : `PosView` (même patron). | ✅ |
