...
| **P6-2** 🟢 **partiel (rc.93/96)** | **Brancher le checkout déclaratif (approche A) + admin** | ✅ **Abonnement** : `UpgradeView` affiche les moyens par marché (rc.93). ✅ **Commercial backend** (rc.96) : `PaymentMethodCatalog` (provider→catégorie) + `POST /api/payments` accepte `provider` (dérive `Payment.method`, trace en réf.) + 5 tests. **Reste** : sélecteur frontend `PosView`/commande consommant l'API. | 🟢 |
...
| **P6-4** 🟢 **adaptateur réf. livré (rc.95)** | *(post-1.0, par marché)* **Premier PSP réel** | ✅ **`FlutterwaveGateway`** (initiate/verify/refund, HTTP mocké, +5 tests) branché sur l'abstraction — **inerte** (flag off + clé vide). **Activation = décision fondateur** : `FLUTTERWAVE_SECRET_KEY` + `PAYMENT_GATEWAYS_ENABLED=true` + secret webhook, puis `mode` du marché `manual`→`auto`. ⚠️ Vérifier contre la doc Flutterwave v3 avant prod. | 🟢 |
