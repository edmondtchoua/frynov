...
|---|---|---|---|
| **P6-1** | **Socle marché↔moyens + mention manuelle** | Table/modèle/migration/seeder `market_payment_methods` (10 marchés, tout `manual`/`quote`) + `GET /api/public/payment-methods`. **Zéro rail, zéro PSP.** Tests : chaque devise renvoie ≥1 moyen ou une mention ; non-régression résolution marché. Docs. | ✅ |
