...
**Adopté** (25 modales / 20 vues) : `CustomerListView`, `PaymentListView`, `SupplierListView`,
...
(création/édition), `MarketplaceListingsView` (connexion marketplace), `OrderDetailView` (enregistrer paiement).
Reste (incrémental, ~4 vues) : ProductForm, PosView, SettingsView, StockList. Contrat testé (`PaymentListView.spec.ts`,
