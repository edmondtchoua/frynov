...
**Adopté** (9 modales / 7 vues) : `CustomerListView`, `PaymentListView`, `SupplierListView`,
`CategoryListView`, `WarehouseView`, `StockAdjustmentView` (création + rejet), `DeliveryListView`
(création + échec). Reste (incrémental, ~17 vues) : Stock, Retours, Promotions, ManualPayment,
RolesPanel, StockTransfer, FiscalPeriod, etc. Contrat testé (`PaymentListView.spec.ts`,
`SupplierListView.spec.ts`, `WarehouseView.spec.ts`, `DeliveryListView.spec.ts` → ouverture d'un
