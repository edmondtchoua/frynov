...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.39`** (cette session → `rc.40+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→14 (rc.35→rc.40).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **26 modales / 21 vues** (rc.19→rc.40) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory, Promotion, Marketplace, OrderDetail, StockList migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~3 vues : ProductForm, PosView, SettingsView). |
