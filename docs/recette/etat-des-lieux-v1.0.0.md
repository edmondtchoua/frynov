...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.36`** (cette session → `rc.37+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→11 (rc.35→rc.37).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **23 modales / 18 vues** (rc.19→rc.37) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory, Promotion migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~6 vues : OrderDetail, MarketplaceListings, ProductForm, PosView, SettingsView, StockList). |
