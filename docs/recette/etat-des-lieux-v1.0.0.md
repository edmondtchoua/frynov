...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.35`** (cette session → `rc.36+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→10 (rc.35→rc.36).
> Tests : **backend 642** (640 ✅ / 2 skipped) · **frontend 242** · `vue-tsc` propre ·
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **22 modales / 17 vues** (rc.19→rc.36) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~7 vues : Promotions, OrderDetail, MarketplaceListings, ProductForm, PosView, SettingsView, StockList). |
...
| 9 | ~~**UX-13 — i18n**~~ | P2 | L | ✅ **Fondation (rc.22) + 4 modules + liste Commandes (rc.24→rc.34)** — noyau i18n léger sans dépendance (FR + EN), API `$t`/`useI18n`, `LanguageSwitcher`, persistance ; **Fournisseurs, Livraisons, Paiements, Rapports + liste Commandes traduits** + libellés `common.*` mutualisés. Voir `docs/modules/i18n.md`. Reste : migration incrémentale des autres modules + langues supplémentaires si besoin. |
