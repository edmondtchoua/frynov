...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.37`** (cette session → `rc.38+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→12 (rc.35→rc.38).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **24 modales / 19 vues** (rc.19→rc.38) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory, Promotion, Marketplace migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~5 vues : OrderDetail, ProductForm, PosView, SettingsView, StockList). |
