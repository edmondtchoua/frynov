...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.40`** (cette session → `rc.41+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→15 (rc.35→rc.41).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **27 modales / 22 vues** (rc.19→rc.41) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory, Promotion, Marketplace, OrderDetail, StockList, ProductForm migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~2 vues : PosView, SettingsView). |
