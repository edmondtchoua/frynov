...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.41`** (cette session → `rc.42+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` vagues 9→16 (rc.35→rc.42).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **29 modales / 23 vues** (rc.19→rc.42) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel, ImportHistory, Promotion, Marketplace, OrderDetail, StockList, ProductForm, PosView (×2) migrées vers `<BaseModal>` (pattern documenté + testé). **Ne reste qu'1 vue** : SettingsView (5 modales). |
