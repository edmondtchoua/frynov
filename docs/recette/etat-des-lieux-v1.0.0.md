...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.31`** (cette session → `rc.32+`).
...
> module Rapports traduit FR/EN (rc.30), `BaseModal` 6ᵉ vague (rc.31), `BaseModal` 7ᵉ vague (rc.32).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **17 modales / 14 vues** (rc.19→rc.32) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~10 vues : Promotions, RolesPanel, StockTransfer, OrderDetail, MarketplaceListings, ImportHistory, ProductForm, PosView, SettingsView, StockList). |
