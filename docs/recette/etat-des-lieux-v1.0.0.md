...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.27`** (cette session → `rc.28+`).
...
> module Paiements traduit FR/EN (rc.28).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **11 modales / 9 vues** (rc.19→rc.27) — Customer, Payment, Supplier, Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~15 vues : Stock, Promotions, ManualPayment, RolesPanel, StockTransfer, FiscalPeriod, OrderDetail…). |
