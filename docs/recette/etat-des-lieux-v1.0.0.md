...
> Mise à jour : 2026-06-07. Branche `release/v1.0.0` à **`v1.0.0-rc.34`** (cette session → `rc.35+`).
...
> liste Commandes traduite FR/EN (rc.34), `BaseModal` 9ᵉ vague (rc.35).
...
| 6 | ~~**UX-03 — adoption `BaseModal`**~~ | P2 | M | ✅ **21 modales / 16 vues** (rc.19→rc.35) — Customer, Payment, Supplier (×2), Category, Warehouse, StockAdjustment (×2), Delivery (×2), Returns, CountryRule, PlanList, StockAlerts, FiscalPeriod (×2), ManualPayment, StockTransfer (×3), RolesPanel migrées vers `<BaseModal>` (pattern documenté + testé). Reste l'adoption incrémentale (~8 vues : Promotions, OrderDetail, MarketplaceListings, ImportHistory, ProductForm, PosView, SettingsView, StockList). |
