...
| **B — Rôles/permissions fins (B2)** | ✅ Retenu — **B2** | 🔄 **En cours** (branche `feature/rbac-phase-b2-custom-roles`). **B2.1 ✅ livré** : `TenantRoleService` + `TenantRoleController` (`/api/workspace/roles`, admin-tenant) — CRUD de rôles custom (scope team `tenant_id`), permissions **bornées par le plan/modules actifs** (`grantablePermissions`), assignation aux membres (`UpdateUserRoleRequest` accepte les rôles custom), isolation testée (`TenantRoleTest`, 6). **B2.2 🔲** : migration des routes sensibles `role:manager\|admin` → `role_or_permission:manager\|admin\|<perm-granulaire>` (préserver le comportement — utiliser les perms granulaires que seul admin/manager possèdent, ex. `products.create`, `inventory.adjust`, PAS les `module.action` que `member` a). **B2.3 🔲** : écran Paramètres → Rôles. |
