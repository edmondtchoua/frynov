  ...
  <div>
    ...
    <template v-else>
      ...
      <div class="card plan-card">
        <div class="plan-card-header">
          <div class="plan-identity">
            ...
            <div>
              <div class="plan-name">{{ subscription?.plan_name ?? $t('billing.unknownPlan') }}</div>
        ...
        <div class="plan-details-grid">
          <div class="plan-detail-item">
            <span class="plan-detail-label">{{ $t('billing.priceMonthly') }}</span>
          ...
          <div class="plan-detail-item">
            <span class="plan-detail-label">{{ $t('billing.priceYearly') }}</span>
            <span class="plan-detail-value">
          ...
          <div class="plan-detail-item" v-if="subscription?.trial_ends_at">
            <span class="plan-detail-label">{{ $t('billing.trialEnd') }}</span>
          ...
          <div class="plan-detail-item" v-if="subscription?.current_period_end">
            <span class="plan-detail-label">{{ $t('billing.renewal') }}</span>
        ...
        <div v-if="trialDaysLeft !== null" class="trial-banner">
          ...
          {{ $t('billing.trialLabel') }} <strong>{{ $t('billing.daysCount', { count: trialDaysLeft }) }}</strong> {{ $t('billing.remaining') }}
        </div>
      ...
      <div class="card" style="margin-top:1rem">
        <h3 class="section-title">{{ $t('billing.usageTitle') }}</h3>

        <div v-if="!subscription" class="empty-state" style="padding:1rem 0">
          <p>{{ $t('billing.noQuota') }}</p>
        </div>
      ...
      <div class="card" style="margin-top:1rem">
        <div class="section-header-row">
          <h3 class="section-title" style="margin-bottom:0">{{ $t('billing.manualPaymentsTitle') }}</h3>
        ...
        <div v-else-if="!payments.length" class="empty-state" style="padding:1.5rem 0">
          ...
          <p>{{ $t('billing.noManualPayments') }}</p>
        </div>
...
const statusLabel = computed(() => {
  const map: Record<string, string> = {
    trialing:         t('billing.subStatus.trialing'),
    active:           t('billing.subStatus.active'),
    suspended:        t('billing.subStatus.suspended'),
    cancelled:        t('billing.subStatus.cancelled'),
    pending_approval: t('billing.subStatus.pending_approval'),
  }
...
function paymentMethodLabel(method: string): string {
  const map: Record<string, string> = {
    mobile_money: t('billing.method.mobile_money'),
    bank_transfer: t('billing.method.bank_transfer'),
    cash: t('billing.method.cash'),
    check: t('billing.method.check'),
  }
...
function paymentStatusLabel(status: string): string {
  const map: Record<string, string> = {
    pending:  t('billing.payStatus.pending'),
    approved: t('billing.payStatus.approved'),
    rejected: t('billing.payStatus.rejected'),
  }
